...
# Filter InstanceNorm warnings about num_features mismatch
warnings.filterwarnings('ignore', message='.*does not match num_features.*', category=UserWarning)

# Kaiwu license authentication (optional)
# Uncomment the following line to enable license authentication
...
args.add_argument('-use_qbm', type=int, default=1, help='0: use BiVAE, 1: use BiQBMVAE')
...
# QBM-VAE specific parameters
args.add_argument('-num_visible', type=int, default=10, help='number of visible nodes in RBM for QBM-VAE')
args.add_argument('-num_hidden', type=int, default=10, help='number of hidden nodes in RBM for QBM-VAE')
args.add_argument('-dist_beta', type=float, default=10.0, help='beta parameter for distribution in QBM-VAE')
...
if __name__ == '__main__':
    ...
    print('training bivae or biqbmvae')
    ...
    if params.use_qbm:
        ...
        bivae = learn_qbmvae(bivae, data_matrix, epochs=100, batch_size=100, lr=0.001, beta_kl=1.0, device=device)
        print('biqbmvae finish training!')
    else:
        print('Using BiVAE model')
